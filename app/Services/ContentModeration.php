...
class ContentModeration
{
    ...
    private function check(?User $user, array $payload, string $context, string $field): void
    {
        ...
        $review = $user === null ? null : ModerationReview::query()->select(['id', 'status', 'created_at', 'categories'])->where('fingerprint', $fingerprint)->first();
        ...
        $review = DB::transaction(function () use ($user, $fingerprint, $context, $payload, $flagged, $field): ModerationReview {
            ...
            // Expired snapshots are removed on the next held submission; no new worker required.
            ModerationReview::query()->where('created_at', '<=', now()->subDays(config('moderation.review_days')))->delete();
            $existing = ModerationReview::query()->select(['id', 'status', 'created_at', 'categories'])->where('fingerprint', $fingerprint)->first();
            if ($existing !== null) {
                return $existing;
            }
    ...
    /** @param array<string, mixed> $payload
     * @return list<string>|null Null means checking was unavailable, not a violation.
     */
    private function classify(array $payload): ?array
    {
        ...
        return array_values(array_filter($policy, fn ($category) => $categories[$category]));
    }
